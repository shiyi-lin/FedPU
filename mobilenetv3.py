...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes=1000, act=nn.Hardswish, in_channels=3, featdim=1280):
        ...
        self.linear3 = nn.Linear(576, featdim, bias=False)
        self.bn3 = nn.BatchNorm1d(featdim)
        self.hs3 = act(inplace=True)
        self.drop = nn.Dropout(0.2)
        self.linear4 = nn.Linear(featdim, num_classes)
        self.init_params()
...
class MobileNetV3_Large(nn.Module):
    def __init__(self, num_classes=1000, act=nn.Hardswish, in_channels=3, featdim=1280):
        ...
        self.linear3 = nn.Linear(960, featdim, bias=False)
        self.bn3 = nn.BatchNorm1d(featdim)
        self.hs3 = act(inplace=True)
        self.drop = nn.Dropout(0.2)
        self.featdim = featdim
